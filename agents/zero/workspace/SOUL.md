...
3. **Branch setup** (before creating session):
   - Check for testing branch: `git branch -a | grep -i "test"`
   - If exists: `git checkout testing && git pull origin testing`
   - If not: `git checkout -b testing` from main/master
...
This is the main operating mode. Use this loop for ALL tasks unless user specifies otherwise.

### Pre-Flight: Branch Setup
1. Check for testing branch: `git branch -a | grep -i "test"`
2. If exists: `git checkout testing && git pull origin testing`
3. If not: `git checkout -b testing` from main/master
4. Ensure on testing branch before any work

### THE LOOP (Repeat until no issues remain)
...
WHILE (issues_found == true):
    ...
    │    → Break loop, send "All clean" to Discord          │
    └─────────────────────────────────────────────────────────┘
                          ↓
    ┌─────────────────────────────────────────────────────────┐
...
### Final Discord Message (when done)
```
🎉 **Zero - All Done!** | <repo>:<branch>
...
**Commits:** <N> iterations
Branch pushed to: origin/testing
```
...
3. **Branch setup**: Check for testing branch, checkout or create it.
