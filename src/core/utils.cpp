...
std::string formatSector(uint64_t sector, uint32_t sector_size) {
    return formatBytes(sector * sector_size);
}
...
bool isValidDevicePath(const std::string& path) {
    ...
    #elif _WIN32
    return startsWith(path, "\\\\.\\PhysicalDrive") || 
           startsWith(path, "\\\\?\\") ||
           startsWith(path, "\\\\.\\");
    #elif __APPLE__
    return startsWith(path, "/dev/");
...
void guidFromString(const std::string& str, uint8_t* guid) {
    ...
    // Reverse the byte order for the first 3 fields (GUID is mixed-endian)
    guid[0] = bytes[3];
    guid[1] = bytes[2];
    guid[2] = bytes[1];
    guid[3] = bytes[0];
